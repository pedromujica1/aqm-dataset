...
pivoted_df.index = pivoted_df.index - pd.Timedelta(hours=3)
...
pivoted_df = pivoted_df.add_prefix(prefix)

# Step 12: Save the modified DataFrame to a file (e.g., CSV)
pivoted_df.to_csv(prefix +'df.csv', index=True)  # Replace 'path/to/save/modified_df.csv' with the actual file path where you want to save the DataFrame
